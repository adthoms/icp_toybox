#pragma once

#include <Eigen/Dense>

inline Eigen::Matrix3d createRotationMatrix(const Eigen::Vector3d& r) {
  Eigen::AngleAxisd rotationX(r[0], Eigen::Vector3d::UnitX());
  Eigen::AngleAxisd rotationY(r[1], Eigen::Vector3d::UnitY());
  Eigen::AngleAxisd rotationZ(r[2], Eigen::Vector3d::UnitZ());
  return (rotationZ * rotationY * rotationX).toRotationMatrix();
}

inline Eigen::Matrix3d skewSymmetric(const Eigen::Vector3d& vec) {
  Eigen::Matrix3d skew;
  skew << 0.0, -vec[2], vec[1], vec[2], 0.0, -vec[0], -vec[1], vec[0], 0.0;
  return skew;
}

inline double GMLoss(double k, double residual_sq) {
  return k / std::pow(k + residual_sq, 2.0);
}

inline double HuberLoss(double k, double residual_sq) {
  if (residual_sq < k * k)
    return 0.5 * residual_sq;
  else
    return k * (std::sqrt(residual_sq) - 0.5 * k);
}
