...
public:
  enum class Type { PointToPlane, GICP };
...
struct PointToPlaneError {
  PointToPlaneError(const Eigen::Vector3d& p_source,
                    const Eigen::Vector3d& p_target,
                    const Eigen::Vector3d& norm_target)
  ...
  create(const Eigen::Vector3d& p_source, const Eigen::Vector3d& p_target, const Eigen::Vector3d& norm_target) {
...
struct GICPError {
  GICPError(const Eigen::Vector3d& p_source,
            const Eigen::Matrix3d& cov_source,
            const Eigen::Vector3d& p_target,
            const Eigen::Matrix3d& cov_target)
  ...
  static ceres::CostFunction* create(const Eigen::Vector3d& p_source,
                                     const Eigen::Matrix3d& cov_source,
                                     const Eigen::Vector3d& p_target,
                                     const Eigen::Matrix3d& cov_target) {
    return (
        new ceres::AutoDiffCostFunction<GICPError, 1, 4, 3>(new GICPError(p_source, cov_source, p_target, cov_target)));
  }
